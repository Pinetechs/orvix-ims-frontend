...
import { useUploadAssetInventoryExcelMutation } from '../../useUploadAssetInventoryExcelMutation.js';
import { IMPORT_COMPLETED_STATUS } from '../../../utils/createInventoryTaskDialogUtils.js';

export function useAssetUploadExcelStep({ wizard }) {
  const fileInputRef = useRef(null);
  const [selectedFile, setSelectedFile] = useState(null);
  const [uploadResult, setUploadResult] = useState(null);

  const uploadMutation = useUploadAssetInventoryExcelMutation();
  ...
  const uploadSelectedFile = async () => {
    ...
    try {
      ...
      wizard.setLocalError(error.message || 'Could not upload asset inventory Excel file.');
      return false;
    }
  };
  ...
  useEffect(() => {
    ...
    if (importJobCompleted) {
      ...
      if (wizard.taskId) {
        queryClient.invalidateQueries({
          queryKey: queryKeys.assetInventory.assetItemsAll(wizard.taskId),
          exact: false,
        });
        queryClient.invalidateQueries({ queryKey: queryKeys.assetInventory.assetLocations(wizard.taskId) });
        queryClient.invalidateQueries({ queryKey: queryKeys.assetInventory.assetCategories(wizard.taskId) });
      }
    }
    ...
    if (importJobFailed) {
      ...
      wizard.setLocalError(importJob?.errorMessage || 'Asset import background job failed.');
    }
  ...
  useEffect(() => {
    ...
    else if (uploadResult || taskImportCompleted) label = 'Review Uploaded Assets';

    wizard.setStepConfig({
      subtitle: 'Select the fixed asset Excel file. The backend validates duplicate barcodes and processes the import in the background.',
